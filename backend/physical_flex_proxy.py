...
def check_args(request: dict) -> dict:
    ...
    Args:
     ...
     Returns:
        args: A dict with all of the parameters containing a flexibility workout regime  if request is valid. Else, returns single-key dict of "failure" with exit code 0
    """
    request_contains_triceps = len(request) == 4 and "triceps stretch" in request.keys()
    triceps_is_int = (
        request_contains_triceps and type(request["triceps stretch"]) == int
    )
    request_contains_bench_press = (
        triceps_is_int and "Neck-and-Shoulder Release" in request.keys()
    )
    bench_press_is_int = (
        request_contains_bench_press
        and type(request["Neck-and-Shoulder Release"]) is int
    )
