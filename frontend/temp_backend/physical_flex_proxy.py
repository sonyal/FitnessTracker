...
def check_args(request: dict) -> dict:
    ...
    """
    request_contains_overhead_press = len(request) == 4 and "triceps stretch" in request.keys()
    overhead_press_is_int = request_contains_overhead_press and type(request["triceps stretch"]) == int
    request_contains_bench_press = overhead_press_is_int and "Neck-and-Shoulder Release" in request.keys()
